...
import { getUserDataUid, deleteComment } from "../../../../Services/firebase";
...
export default function CommentsTile({ docId, item, uid, comment_id }) {
  ...
  const commentId = item.docId;
  const postId = docId;

  const handleDeleteComment = () => {
    deleteCommentMutation.mutate({ uid, postId, commentId });
  };
      ...
      {custom_user.uid === item?.uid ? (
        ...
        >
          <svg
            ...
            strokeWidth="1.5"
            stroke="currentColor"
            className="stroke-red-500 disabled:opacity-25 w-5 h-5"
          >
            <path
              strokeLinecap="round"
