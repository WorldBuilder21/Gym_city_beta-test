...
export default function EditWorkoutRoutine() {
  ...
  const [difficulty, setDifficulty] = useState(routineData.difficulty);
  ...
  const onSubmit = async (data, e) => {
    ...
    try {
      ...
      const docRef = doc(
        db,
        `users/${custom_user.uid}/routines/${routineData.docId}`
      );
      if (imageFile !== null) {
        const fileRef = ref(storage, routineData.photoUrl);
      ...
      if (title !== routineData.title) {
        await updateDoc(docRef, {
          title: title,
        });
      }
      if (bio !== routineData.bio) {
        await updateDoc(docRef, {
          bio: routineData.bio,
        });
      }
      if (difficulty.name !== routineData.difficulty.name) {
        await updateDoc(docRef, {
        ...
        <div className="w-full px-5 max-w-md md:max-w-lg flex flex-col">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 mt-5">
            <div className="space-y-1">
              <Controller
                name="title"
                defaultValue={routineData.title}
                control={control}
                rules={{
                  maxLength: {
                    value: 60,
