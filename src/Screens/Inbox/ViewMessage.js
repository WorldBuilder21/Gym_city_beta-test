...
export default function ViewMessage() {
  ...
  const {
  ...
  } = useInfiniteQuery(
    {
      queryKey: ["replies"],
      queryFn: (pageParam) =>
        viewReplies(message.message.id, pageParam.pageParam),
  ...
  const {
  ...
  } = useQuery(
    {
      ...
      queryFn: () => getRepliesCount({ docId: message.message.id }),
    },
    { enabled: false }
  );
    ...
    <div>
      ...
      <div className="flex flex-col m-5">
        <div className="flex flex-col">
            ...
            <div className="flex flex-col justify-center ml-3">
              <div className="flex items-center">
                <p className="font-semibold mr-2 truncate">
                  {message.user.usertype === "Gym"
                    ? message.user.gymname
          ...
          <RecieverCard recieverId={message.message.recieverId} />
          <div className="mt-4 font-semibold text-2xl">
            {message.message.title}
          </div>
          <p className="mt-2">{message.message.body}</p>
          {message.message.attachments.length > 0 ? (
            <div className="flex flex-col">
              <span className="flex items-center text-slate-500 text-lg mt-3">
                ...
                <span>Attachments • {message.message.attachments.length}</span>
              </span>
              <div className="flex flex-wrap lg:space-x-2">
                {message.message.attachments.map((data, index) => {
