...
import { editGoalDoc } from "../../../Services/firebase";
...
export default function EditGoalDialogBox({
  isOpen,
  Fragment,
  handleClose,
  refetch,
  ...
  //console.log("ts:", formatted_deadline);

  const editGoalMutation = useMutation({
    mutationFn: editGoalDoc,
    onSuccess: (data) => {
      setIsLoading(false);
      handleClose();
      reset();
  ...
  const onSubmit = async (data, e) => {
    ...
    const { new_currentWeight, new_target_weight, new_deadline } = getValues();
    setIsLoading(true);
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  >
                    ...
                    <Controller
                      control={control}
                      name="deadline"
                      defaultValue={formatted_deadline}
                      rules={{
                      ...
                      render={({
                        ...
                        <DatePicker
                          ...
                          onChange={onChange}
                          value={pickedDate}
                          defaultValue={dayjs(formatted_deadline)}
                          label="Deadline"
                          views={["month", "year"]}
                          slotProps={{
                            textField: {
